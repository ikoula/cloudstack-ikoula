<?php
	...
	//Première instance :
		...
		print("Parametrage de la premiere instance (MySQL maitre).\n");
		...
		Pour ce script nous utiliserons la m1.small mais voici ce qui est actuellement disponible :
		...
		$vm01['conf']['templateid'] = "06239517-eb64-441a-ae71-7a0c6d3a057a";
			...
			$mysqlreplpwd = trim (fgets(STDIN));
			// Le port SSH depuis lequel vous pourrez vous connecter sur cette instance via l'adresse ip publique
			print("Port SSH publique a utiliser pour cette instance : ");
			$vm01['conf']['sshpubport'] = trim(fgets(STDIN));

		/* Données utilisateur a passer au processus de déploiement de l'instance.
		Ce paramètre nous sert à passer les paramètres Puppet au travers des userdatas */
		$vm01['conf']['userdata'] = "echo $mysqlrootpwd $mysqlreplpwd > /tmp/mysql && puppet module install ikoula/mysqlmaster && puppet apply -e 'include mysqlmaster'";
...
/* ------------------------------ NE PLUS RIEN MODIFIER APRES CETTE LIGNE ----------------------------------- */
	...
	// On memorise aussi l'IP privee de la première interface pour un usage ulterieur
	$mysqlmasterip = $json_response['queryasyncjobresultresponse']['jobresult']['virtualmachine']['nic']['0']['ipaddress'];
...
/*-------------------------------- Création des règles de sortie --------------------------------*/
	...
	##########################################################
	# Ouverture du port public pour le SSH de la 1e instance #
	##########################################################
	// Requête API
	...
	#####################################################
	# Redirection de port pour le SSH de la 1e instance #
	#####################################################
	// Requête API
	...
	$args['publicport'] = $vm01['conf']['sshpubport'];
	...
	print("Parametrage de la seconde instance (MySQL slave).\n");
	
	print("Nom a donner a votre instance : ");
	$vm02['conf']['hostname'] = trim(fgets(STDIN));
	print("Port SSH publique a utiliser pour cette instance : ");
	$vm02['conf']['sshpubport'] = trim(fgets(STDIN));
	$vm02['conf']['userdata'] = "echo $mysqlrootpwd $mysqlreplpwd $mysqlmasterip > /tmp/mysql && puppet module install ikoula/mysqlslave && puppet apply -e 'include mysqlslave'";

	###################################
	# Creation de la seconde instance #
	###################################

	// Requête API
	$args['command'] = "deployVirtualMachine";
	$args['zoneid'] = $vm01['conf']['zoneid']; //Meme zone que la 1e instance
	$args['serviceofferingid'] = $vm01['conf']['serviceofferingid']; //Meme ressources que la 1e instance
	$args['templateid'] = $vm01['conf']['templateid']; //Meme template que la 1e instance
	$args['networkids'] = $vm01['conf']['networkid']; //Meme network que la 1e instance
	...
	###########################################################
	# Ouverture du port public pour le SSH de la 2de instance #
	###########################################################
	// Requête API
	$args['command'] = "createFirewallRule";
	$args['ipaddressid'] = $vm01['conf']['publicIPid'];
	...
	// Le port de début (port ssh public qui sera plus tard redirigé vers le port 22(ssh) de votre VM)
	$args['startport'] = $vm02['conf']['sshpubport'];

	// Le port de fin (port ssh public qui sera plus tard redirigé vers le port 22(ssh) de votre VM)
	$args['endport'] = $vm02['conf']['sshpubport'];
	...
	######################################################
	# Redirection de port pour le SSH de la 2de instance #
	######################################################
	// Requête API
	$args['command'] = "createPortForwardingRule";
	$args['ipaddressid'] = $vm01['conf']['publicIPid'];
	...
	// Le Port public sur lequels vous allez initialiser la connection et définie plus tôt comme ouvert
	$args['publicport'] = $vm02['conf']['sshpubport'];
