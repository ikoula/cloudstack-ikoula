...
$templateuuid = trim(fgets(STDIN));
        ...
        //Première instance :
                ...
                */
                ...
                $vm01['conf']['userdata'] = "echo $pwmysqlroot $pwmysqldrupal7 > /tmp/mysql && puppet module install ikoula/drupal && puppet apply -e \"include drupal\"";
...
/*-------------------------------- Création des règles de sortie --------------------------------*/
        ...
        // Le port de début (port pubssh port qui sera plus tard redirigé vers le port 22(ssh) de votre VM)
        $args['startport'] = $pubssh;

        // Le port de fin (port pubssh port qui sera plus tard redirigé vers le port 22(ssh) de votre VM)
        $args['endport'] = $pubssh;
